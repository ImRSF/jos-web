...
import { loadJRTable, jrTableChild } from "./table-job-request.js";
...
const getFormValues = (jrNumber) => {
  ...
  $.ajax({
    ...
    success: data => {
      ...
      $("#inph-requestedBy").val(jobRequestDetails.RequestedBy);
      // Input Fields 
      ...
      //$("#inpt-joNumber").val(jobRequestDetails.JobNumber);
      ...
      $("#inpt-requestedBy").val(
        `${jobRequestDetails.RequestedByLastName}, ${jobRequestDetails.RequestedByFirstName}`
      );
      $("#inpt-requestedByWorkplace").val(
        `${jobRequestDetails.CompanyInitial}-${jobRequestDetails.DepartmentInitial}--${jobRequestDetails.SectionDescription}`
      );
...
const isLastJOReviewed = async (employeeNo) => {
  ...
  return await ajaxCaller(AJAX_REQ_GET_LAST_JO_REVIEWED);
};

const addJobReview = (addReviewProps) => {
  $.ajax({
    url: "/jobs/add-review",
    method: "POST",
    data: {
...
$(document).ready(function () {
  ...
  $("#table-job-request tbody").on("click", "td.details-control", function () {
    ...
    if (row.child.isShown()) {
      ...
      row.child(jrTableChild(row.data())).show();
      tr.addClass("shown");
    }
  });
...
$(document).on("click", ".viewForm", function () {
  ...
  $.ajax({
    url: "/jobs/job-request",
    method: "GET",
    data: { requestNumber: rowRequestNumber },
    success: data => {
      const jobRequestDetails = data.jobRequestList[0];
      console.log(jobRequestDetails)
      // Hidden Inputs
      $("#inph-requestId").val(jobRequestDetails.RequestID);
      $("#inph-requestorId").val(jobRequestDetails.Requestor);
      $("#inph-requestedByCompany").val(jobRequestDetails.CompanyID);
...
$(document).on("show.bs.modal", "#modal-employee-list", function () {
  employeeTable = loadEmployeeTable("SectionID", user.SectionID);
  ...
  employeeTable.on("click", "tr", function () {
    ...
    $("#inpt-contactNo").val(removeSpecialCharacters(employeeData.ContactNo));
    $("#inpt-requestedByWorkplace").val(
      `${employeeData.CompanyInitial}-${employeeData.DepartmentInitial}--${employeeData.SectionDescription}`
    );
...
$(".confirmAction").on("click", function (e) {
  ...
  validateFieldByInputLength(
    "inpt-subject",
    50,
    "This field can only support 50 characters and below."
  );
  validateFieldByInputLength(
    "txt-description",
    435,
    "This field can only support 435 characters and below."
  );
  //validateCategory();
  if ($(".validation-error").length != 0) {
    alert(
      "There are fields that failed the validation rules. Please check the fields marked in red."
    );
    return;
  } else {
...
$("#btn-addJobRequest").on("click", function () {
  ...
  $("#inpt-requestedByWorkplace").val(
    `${user.CompanyInitial}-${user.DepartmentInitial}--${user.SectionDescription}`
  );
  $("#inpt-contactNo").val(removeSpecialCharacters(user.ContactNo));
  $("#inpt-statusName").val("");
  $("#t-statusName").text("Approved");
  $("#inpt-category").removeAttr("readonly");
  $("#inpt-subject").removeAttr("readonly");
...
$("#img-viewJODetailedReport").on("click", function () {
  viewJODetailedReport($("#inpt-joNumber").val());
});
...
$("#inpt-subject").on("input", function () {
  ...
  if ($(this).val().length >= charLimit) {
    $(
      `<sub id="text-counter" class="text-danger validation-error">This field can only accept 50 characters or less</sub>`
    ).insertAfter($(this));
    $(this).val(
      $(this)
        .val()
    ...
    $(
      `<sub id="text-counter" class="text-success">You only have ${
      $(this).val().length
...
$("#txt-description").on("input", function () {
  ...
  if ($(this).val().length >= charLimit) {
    $(
      `<sub id="text-counter" class="text-danger validation-error">This field can only accept 430 characters or less</sub>`
    ).insertAfter($(this));
    $(this).val(
      $(this)
        .val()
