...
(function () {
  ...
  $("#btn-submit-jobOrder").on("click", function () {
    ...
    jobItemTable.rows().every(function () {
      ...
      getJobItems = jobItems.map(function (element) {
        let objVal = _objectSpread({}, element);
      ...
      success(data) {
        switch (data.jobOrder.userGroupName) {
          ...
          case "SQA":
            ...
            $("#btn-submit-addJobItems").remove();
            $("#btn-submit-clearJobItems").remove(); // NOTICE: Guide For Job Review
  ...
  const getJobItems = function getJobItems() {
  ...
  const getPriority = function getPriority() {
    let priorityList = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    let priorityID = arguments.length > 1 ? arguments[1] : undefined;
    $("#sel-joPriority").empty();
    let obtainedData = "";
    let getPriorityList = priorityList.map(function (element) {
      return {
        PriorityName: element.PriorityName,
        PriorityID: element.PriorityID
      };
    });
    getPriorityList.filter(function (val) {
      return val.PriorityName !== null && val.PriorityName !== undefined && val.PriorityName !== val.PriorityName;
    });
    getPriorityList.forEach(function (element) {
      obtainedData += "<option value=" + element.PriorityID + ">" + element.PriorityName + "</option>";
    });
  ...
  const getStatus = function getStatus() {
    let statusList = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    let statusID = arguments.length > 1 ? arguments[1] : undefined;
    $("#sel-joStatus").empty();
    let obtainedData = "";
    let getStatusList = statusList.map(function (element) {
      return {
        StatusName: element.StatusName,
        StatusID: element.StatusID
      };
    });
    getStatusList.filter(function (val) {
      return val.StatusName !== null && val.StatusName !== undefined && val.StatusName !== val.StatusName;
    });
    getStatusList.forEach(function (element) {
      obtainedData += "<option value=" + element.StatusID + ">" + element.StatusName + "</option>";
    });
    $("#sel-joStatus").append(obtainedData);
    $("select[id='sel-joStatus'] option[value='" + statusID + "']").attr('selected', 'selected');
  };

  const defaultValues = function defaultValues(formDetails) {
    $("#txt-details").val(""); // TODO: Refactor these 5 functions!
  ...
  function getAttachedFilesForJO() {
