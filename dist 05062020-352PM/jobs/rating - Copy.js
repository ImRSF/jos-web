...
function markStarsAsActiveB(index, id) {
  ...
  for (var i = 0; i <= index; i++) {
    ...
    switch (index) {
      case '0':
        $($stars.get(i)).find("[data-reviewId=" + id + "]").addClass('fa-angry'); // $("#rateMe").data("rating", 1)

        $(".comment").data("rating", 1);
        break;

      case '1':
        $($stars.get(i)).find("[data-reviewId=" + id + "]").addClass('fa-frown'); // $("#rateMe").data("rating", 2)

        $(".comment").data("rating", 2);
        break;

      case '2':
        $($stars.get(i)).find("[data-reviewId=" + id + "]").addClass('fa-meh'); // $("#rateMe").data("rating", 3)

        $(".comment").data("rating", 3);
        break;

      case '3':
        $($stars.get(i)).find("[data-reviewId=" + id + "]").addClass('fa-smile'); // $("#rateMe").data("rating", 4)

        $(".comment").data("rating", 4);
        break;

      case '4':
        $($stars.get(i)).find("[data-reviewId=" + id + "]").addClass('fa-laugh'); // $("#rateMe").data("rating", 5)
  ...
  $('#rateMe').on('click', '#voteSubmitButton', function () {
    ...
    $.ajax({
      ...
      data: {
        ...
        reviewDesc: $("#review-Description").val(),
        reviewVal: $("#rateMe").data("rating")
      },
