...
exports.getJobRequests = async (req, res) => {
  try {
    ...
    if (req.xhr) {
      const jobRequestList = {
        data: await JobRequest.getJobRequestByUserAndStatus(req.query.personnelID, req.query.statusID)
      };
      return res.send(jobRequestList);
    }
...
exports.getJobRequest = async (req, res) => {
  if (req.xhr) {
    return res.send({
      jobRequestList: await JobRequest.getJobRequest("RequestNumber", req.query.requestNumber),
      personnelID: req.session.personnelID,
      attachedFile: await Misc.getAttachedFile("JobRequest", req.query.requestNumber)
    });
  }
...
exports.postJobRequests = async (req, res) => {
  ...
  await Misc.insertFiles(request.requestNumber, null, req.files["file-jobRequest"]);
